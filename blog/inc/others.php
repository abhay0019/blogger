...
<div class="container-fluid">
	...
	<div class="container-fluid" style="border-bottom:1px solid grey;box-shadow: 512px 125px 120px rgba(0,0,0,.4);">
		<?php
			$query="select * from others order by id desc";
			$result=mysqli_query($con,$query);
			if(mysqli_num_rows($result)>0)
			{
				$row=mysqli_fetch_assoc($result);
				echo '<img src="img/others/'.$row['image'].'" class="img-responsive">';	
			}
			else
			{
				echo '<img src="img/others/main.jpg" class="img-responsive">';
			}
			?>
	...
	<?php
		...
		if(mysqli_num_rows($result)>0)
		{
			...
			while($row=mysqli_fetch_assoc($result))
			{
		        ...
		        <a data-id="'.$row['id'].'" data-type="others" data-backdrop="static" data-toggle="modal" href="#content-modal">
		        <span class="mov-story">
		        <div class="panel panel-default">
				    <div class="panel-body zero-padding">
				        <img src="img/others/'.$row['image'].'" class="img-responsive item-img">
